...
const CommonFormPTW = ({ formName,getApprovalsData , setDriveFormModel , setPtwModalHead ,formData }) => {
  const {
    register,
    handleSubmit,
    SubmitTask,
  ...
  const handleAddActivity = async (data, formType) => {
    ...
    try {
      ...
      if (!["photo", "ptwphoto", "teamdetails", "vehicle"].includes(which)) {
        ...
        const url = `/submit/ptw/${formType}/${which}${sessionStorage.getItem("opid") ? `/${sessionStorage.getItem("opid")}` : ""
          }`;
