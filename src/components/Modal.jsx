...
const Modal =   ({ size, modalHead = "", children, isOpen, setIsOpen, closeButton = false }) => {
    ...
    return (
        <div className={isOpen ? 'z-[4000] flex justify-around place-items-center bg-white dark:bg-darkBg fixed rm-scroll overflow-hidden top-0 bottom-0 right-0 left-0' : 'hidden'} style={{ background: 'rgba(6, 6, 6, 0.9)' }} onClick={() => { setIsOpen(prev => !prev) }}>
            <div onClick={(e) => e.stopPropagation()} className={`relative bg-lightBg dark:bg-darkBg  ${sizeType[size]} rounded-md modal-inner`}>
                ...
                {/* <div className='  overflow-scroll p-2'>  real*/} 
                <div className=''>
                    {children}
                </div>
                {closeButton &&
