...
const NewMultiSelects = ({ label, option = [], value = [], required = false, placeholder = "Select...", cb = (data) => { }, ...props }) => {
    ...
    return (
        <div className={`max-w-[170px] min-w-[100px]  relative p-0  ${props?.height || ''} ${props?.className || ''} w-full`}>
            ...
            <MultiSelect
                className="outline-none font-semibold rounded-md border-main mt-[4px] border-solid border-[#64676d] border-[1.2px]"
                options={option}
                value={value}
